...
#import "GMMobileUtilsShareInternal_ios.h"
...
@interface GMMobileUtilsShareInternal ()
{
    gm::runtime::DispatchQueue __dispatch_queue;
    id<GMMobileUtilsShareInterface> __impl;
}@end


@implementation GMMobileUtilsShareInternal
...
+ (void)load
{
    ...
    Class base = [GMMobileUtilsShareInternal class];
...
- (instancetype)init
{
    ...
    if (self)
    {
        __impl = (id<GMMobileUtilsShareInterface>)self;
    }
    return self;
}
- (double)__EXT_NATIVE__mobile_utils_share_open:(char*)__arg_buffer arg1:(double)__arg_buffer_length
{
    ...
    std::string_view mime = gm::wire::codec::readValue<std::string_view>(__br);
    ...
    [__impl mobile_utils_share_open:title mime:mime value:value callback:callback];
